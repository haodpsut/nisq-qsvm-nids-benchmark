...
corrected one. Two consequences of that choice are stated so that the tally is
read correctly. The entanglement family has one member, so for the $20$ cells
of the regime map that compare \QSVM{} with QSVM-\Zmap{} the corrected $p$ is
the raw $p$; and the map's $110$ cells fall into about $56$ families, so under
a global null the expected number of false verdicts across the map is about
$2.8$. Sec.~\ref{sec:regimemap} therefore also reports the tally under a single
Benjamini--Hochberg correction over all $110$ cells. A comparison is called
favourable to one side only when the corrected $p<0.05$ \emph{and} the
confidence interval excludes zero; otherwise it is inconclusive, and we report
it as such rather than as a win.
\end{revbody}
...
\texttt{audit\_prose.py} (134) re-derives every number written in the text of
